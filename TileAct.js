...
function createMatrix(n) {
	var board = document.getElementById("board"); // 게임 보드판
	for (var i = 0; i < n; i++){
		var t = document.createElement('article'); // article 태그 == tile 생성 
...
function Start() {
	var tilesNum; // 총 타일 수
	var board = document.getElementById("board"); // 게임 보드판
	var sboard = document.getElementById("sub-board"); // 서브 보드판(메뉴판)
	var select = document.getElementById("select"); // 처음 화면(난이도 선택 화면)
	var play = document.getElementById("play"); // PLAY 버튼
	var Continue = document.getElementById("continue"); // CONTINUE 버튼
	var tileset = document.getElementsByName("tileset"); // 라디오 버튼들
	var Count = document.getElementById("count"); // 남은 타일 개수
	for(var i = 0; i<tileset.length; i++){ // 라디오 버튼 개수 만큼 반복
	...
	var boardsize = Math.sqrt(tilesNum) * 90; // 타일 개수 별 보드판 크기 설정
...
function tileE(clicked_id) {
	var clickedTile = document.getElementById(clicked_id); // 클릭한 타일 id 불러옴
	var Continue = document.getElementById("continue");
	var total = document.getElementById("score");
	var Count = document.getElementById("count");
	for (var i = 0; i < ansId.length; i++){ // 정답 타일 개수만큼 반복
...
//랜덤한 타일 Id 선택
function getTileID() {
	var selectID = []; // 랜덤한 타일 Id 저장
	for(var i = 0; i < tiles; i++) {
		var num = RandomNumber(alltiles);
...
function printTile(t) {
	for(var i = 0; i < t.length; i++){ // 정답 타일 수 만큼 반복
		var answer = document.getElementById(t[i]); // t에 해당하는 id 얻어옴
...
function hideTile(t) {
	for(var i = 0; i < t.length; i++){ // 정답 타일 수 만큼 반복
		var answer = document.getElementById(t[i]); // 정답 타일의 id 가져오기
		answer.setAttribute("class", "tile"); // 처음 상태의 타일로 설정
	}
}
//플레이 버튼
function Play() {
	var delay = [];// 출력 중 클릭 못하게 하기 위한 delay배열
...
function Restart() {
	...
	for(var i = 0; i < alltiles; i++){ // 모든 타일 회색으로 초기화
...
function Back() {
	...
	var sboard = document.getElementById("sub-board");
	...
	if(play.style.display == "none" && Continue.style.display == "none") {} // 시작하면 끝날때까지 처음으로 못감
		...
		for (var i = 0; i < alltiles; i++) {
			var t = document.getElementById(i); // 타일 id 불러옴
